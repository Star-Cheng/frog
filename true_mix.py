...
def pcgpa_reconstruction_algorithm(FROG_Trace_Clean, t, tau, E_est, omega_0, dt, Max_Iter=100, use_experimental_data=True):
    ...
    for k in range(Max_Iter):
        ...
        if k % 10 == 0 and k > 5:
            ...
            # 计算频谱重心
            omega_axis_temp = np.fft.fftshift(np.fft.fftfreq(N, dt) * 2 * np.pi) + omega_0
...
def post_process_reconstructed_pulse(E_rec, t, dt, f0, c_nm_per_fs):
    ...
    f_center = f0  # 使用估计的中心频率

    intensity_freq = np.abs(Ew_rec) ** 2
    intensity_freq_sum = np.sum(intensity_freq)

    if intensity_freq_sum > 0:
        spec_com_rec = np.sum(f_axis * intensity_freq) / intensity_freq_sum
        freq_offset = spec_com_rec - f_center

        # 校正频率偏移
        if np.abs(freq_offset) > 0.01 * f_center:
            E_rec = E_rec * np.exp(-1j * 2 * np.pi * freq_offset * t)
            print(f"频率偏移校正: {freq_offset:.4f} 1/fs")
            Ew_rec = np.fft.fftshift(np.fft.fft(np.fft.ifftshift(E_rec)))
    else:
    ...
    if intensity_freq_final_sum > 0:
        f_mean = np.sum(f_axis * intensity_freq_final) / intensity_freq_final_sum
        lambda_calc = c_nm_per_fs / f_mean
    else:
        lambda_calc = c_nm_per_fs / f0
        print("警告: 频谱强度总和为0，使用估计的中心频率计算波长")
...
def reconstruct_from_experimental_data(data_file="frog_data1 2.xlsx", Max_Iter=100):
    ...
    # 计算SHG中心波长
    omega_SHG = 2 * omega_0 + np.fft.fftshift(np.fft.fftfreq(N_actual, dt_fs) * 2 * np.pi)
    lambda_SHG = 2 * np.pi * c_nm_per_fs / omega_SHG
    ...
    if SHG_intensity_sum_total > 0:
        lambda_SHG_mean = np.sum(lambda_SHG * SHG_intensity_sum) / SHG_intensity_sum_total
    else:
        lambda_SHG_mean = lambda_calc / 2
